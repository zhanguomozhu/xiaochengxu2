...
class Image {
     ...
     * @param  integer $mode       [类型 0 字母 1 数字 其它 混合]
     * @param  string  $type       [图像格式]
     * @param  integer $imageL     [宽]
     * @param  integer $imageH     [高]
     * @param  string  $verifyName [验证码名称]
     * @param  integer $fontSize   [字体大小]
     * @param  string  $fontttf    [字体库]
     * @return [type]              [description]
     */
    static function buildImageVerify1($length=5, $mode=1, $type='png', $imageL=0, $imageH=0, $verifyName='verify',$fontSize=25,$fontttf='') {  // 验证码字体，不设置随机获取  抄袭的 onethink
        import('ORG.Util.String');
        //验证码字符
        $codeSet = String::randString($length, $mode);
      
        //干扰点字符
        $codeSet1= String::randString(6, $mode);
        ...
        for($i = 0; $i < 10; $i++){
            ...
            for($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring($im, 5, mt_rand(-10, $imageL),  mt_rand(-10, $imageH), $codeSet1[mt_rand(0, 6)], $noiseColor);
            }
        }
        // 绘干扰线
        $px = $py = 0;
        ...
        for ($i = 0; $i<$length; $i++) {
            $codeNX += mt_rand($fontSize, $fontSize*1.3);
            // 写一个验证码字符
            imagettftext($im, $fontSize, mt_rand(-40, 40), $codeNX, $fontSize*1.5, $color, $fontttf, $codeSet[$i]);
        }

        // 保存验证码
        session($verifyName, md5(strtolower($codeSet)));
